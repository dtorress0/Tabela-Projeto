...
function adicionarEncomendaNaTabela(encomenda) {
    ...
    celulaValor.textContent = formatarParaMoeda(encomenda.valor_unitario);
    novaLinha.appendChild(celulaValor);

    var celulaTotal = document.createElement('td');
    celulaTotal.textContent = formatarParaMoeda(calculaTotal(parseFloat(encomenda.qtde), parseFloat(encomenda.valor_unitario)));
...
function calculaTotal(quantidade, valorUnitario) {
